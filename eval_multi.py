        ...
        with torch.no_grad():
            ...
            if hi >= args.crop_size:
                ...
                for b_idx in range(bi):
                    ...
                    #cur_output, _ = model(cur_input)
                    _, cur_output = model(cur_input)
                ...
                #output_scattered, _ = model(input_crops)
                _, output_scattered = model(input_crops)
...
def get_net():
    ...
    net = network.get_net(args, criterion=None, tasks=tasks)
    if args.inference_mode == 'pooling':

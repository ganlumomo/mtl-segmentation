...
def make_test_dataset(quality, mode, maxSkip=0, cv_split=0):
    ...
    for it in c_items:
        ...
        items.append(item)
    logging.info('TartanAir has a total of {} test images'.format(len(items)))
...
class TartanAir_Semantic(data.Dataset):
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None,
        ...
        if self.class_uniform_pct > 0:
            if self.scf:
                json_fn = 'tartanair_semantic_tile{}_cv{}_scf.json'.format(self.class_uniform_tile, self.cv_split)
            else:
                json_fn = 'tartanair_semantic_tile{}_cv{}_{}_hardnm{}.json'.format(self.class_uniform_tile, self.cv_split, self.mode, self.hardnm)
